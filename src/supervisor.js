...
import { hasValidNodeModules } from './npm-public.js';
...
class Supervisor {
  ...
  async bootService(mod) {
    ...
    if (env.SESSIONS_PATH) {
      await fsp.mkdir(path.resolve(mod.absDir, env.SESSIONS_PATH), { recursive: true });
    }
  ...
  async prepareCommand(mod) {
    ...
    const hasDeps = await hasValidNodeModules(mod.absDir);
  ...
  async moduleEnv(mod) {
    ...
    if (mod.port && (env.PORT == null || String(env.PORT).trim() === '')) {
      env.PORT = String(mod.port);
    }
    return env;
  }
  ...
  async waitUntilReady(mod) {
    ...
    if (ready.url) {
      ...
      const probeMs = Number(ready.probeTimeoutMs || 2000);
      ...
      while (Date.now() - startedAt < timeoutMs) {
        ...
        try {
          const response = await fetch(ready.url, { signal: AbortSignal.timeout(probeMs) });
          // сервер поднят, даже если корень отдаёт 404 (типично для API)
          if (response.status > 0) {
            return;
          }
        } catch {
          // still booting / event-loop busy
        }
