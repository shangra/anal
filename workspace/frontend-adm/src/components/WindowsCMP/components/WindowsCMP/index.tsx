...
import { WindowCMP } from '../WindowCMP';
import { WindowSidebar } from '../Sidebar';
import { ActiveWindowPanel } from '../ActiveWindowPanel';
import style from '../../windowsCmp.module.css';
import { isEmptySidebar } from '../../utils';
...
export class WindowsCMP extends Component<IProps, IState> {
          ...
          windows: {
            ...
            [key]: {
              ...
              style: {
                ...
                transform: `translate(${originalPosition?.x || 0}px, ${originalPosition?.y || 0
                  }px)`,
              },
            },
          },
      ...
      this.setState(
        (prevState) => {
          ...
          return {
            ...
            sidebars: {
              ...
              ...(!windows.position && {
                [PanelPosition.bottom]: {
                  ...
                  activeTabId: sidebars[PanelPosition.bottom].activeTabId,
                },
              }),
              ...(windows.position && {
                [windows.position]: {
            ...
            activeTabId:
              key === prevState.activeTabId ? '' : prevState.activeTabId,
          };
        },
      );
    } else {
  ...
  setTabActive = (position: PanelPosition) => (key: string) => {
    if (position === PanelPosition.bottom) {
      this.setState(
        (prevState) => {
          const sidebars = {
            ...
            [PanelPosition.left]: {
              ...
              tabs: { ...prevState.sidebars[PanelPosition.left].tabs },
            },
            [PanelPosition.right]: {
              ...prevState.sidebars[PanelPosition.right],
              tabs: { ...prevState.sidebars[PanelPosition.right].tabs },
            },
          };
        ...
        () => {
          const w = this.state.windows[key] as WindowWithTray | undefined;
          if (!w) return;
   ...
   * @param key {string} - id окна
   * @description Модальное окно скрывается и появляется в садйбаре и области активного окна
   */
  ...
  render() {
    ...
    const isBottomSidebarHasTabs =
      Object.keys(this.state.sidebars[PanelPosition.bottom].tabs).length > 0;

    return (
      <div className={style.windowSystemContainer}>
        {Object.entries(this.state.sidebars).map(([position, sidebar]) => {
          ...
          return (
            <div className={style.windowSystemContent}>
              ...
              {showActivePanel && (
                <ActiveWindowPanel
                  isBottomSidebarOpen={isBottomSidebarHasTabs}
                  windows={Object.values(sidebar.tabs)}
                  ...
                  onChangePanelPosition={this.onChangePanelPosition}
                />
              )}
            </div>
          );
          ...
          (window) =>
            !window.isMinimized && (
              <WindowCMP
                ...
                isFullscreen={!!this.state.fullscreenWindows[window.key]}
              >
                {window.content}
              </WindowCMP>
            ),
