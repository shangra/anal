...
const SelectableMixin = {
    async *fsck_self(self, opts) {
        ...
        yield FSCK.assert(
            path,
            `Объект метаданных содержит ожидаемую структуру (manifest.settings)`,
            self_settings && 'object' == typeof self_settings,
            true,
            { self }
        );
        ...
        try {
            yield* new ConnectorClass().fsck_self(connectorData, {
                ...opts,
                path,
            });
        } catch (error) {
        ...
        const self_settings_sqlalias = String(
            self_settings.sqlalias || ''
        ).trim(); // "сложный запрос" (по факту, это "inline view")
        ...
        yield FSCK.assert(
            path,
            `В настройках метаданных есть физическая таблица или сложный запрос`,
            has_phys || has_view,
            true,
            { self }
        );
        ...
        if (has_view) {
            ...
            yield FSCK.info(
                path,
                `Алиас "сложного запроса"`,
                has_phys ? self_settings_table : null,
                null,
                { self }
            );
        ...
        if (self_settings_filter.length) {
            ...
            yield FSCK.assert(
                path,
                `Нет ошибки при обработке свойства "Фильтр" как JSON (manifest.settings.filter)`,
                fail,
                null,
                { self }
            );
            ...
            yield FSCK.assert(
                path,
                `Свойство "Фильтр" объекта метаданных содержит ожидаемую структуры (manifest.settings.filter должен быть объектом)`,
                'object' == typeof self_settings_filter,
                true,
                { self }
            );
        } else {
        ...
        yield FSCK.info(path, `Наличие применяемого "фильтра"`, has_filt, {
            self,
        });
        ...
        yield FSCK.info(
            path,
            `Источник данных для формирования полей`,
            cte_base_alias,
            { self }
        );
        ...
        if (skip_read_test) {
            yield FSCK.tell(
                path,
                `Тестирование реального чтения данных не проводилось`,
                { self },
                { type: 'warn' }
            );
        } else {
            ...
            yield FSCK.check(
                path,
                `Нет ошибки при тестовом чтении данных`,
                fail,
                FSCK.is.not_error,
                { self },
                { failtype: 'fail' }
        ...
        if (1) {
            ...
            yield FSCK.check(
                path,
                `Нет ошибки при выгрузке слепка метаданных`,
                fail,
                FSCK.is.not_error,
                { self },
                { failtype: 'fail' }
        ...
        if (1) {
            ...
            yield FSCK.check(
                path,
                `Нет ошибки при выгрузке слепка метаданных со ссылками`,
                fail,
                FSCK.is.not_error,
                { self },
                { failtype: 'fail' }
