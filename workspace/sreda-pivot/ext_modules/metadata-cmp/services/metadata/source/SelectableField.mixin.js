...
const SelectableFieldMixin = {
    async * fsck_self(self, opts) {
        ...
        yield FSCK.check(FSCK.path_prop(path, "name"), `Поле имеет непустое свойство name`,
            !!String(self.manifest.name ?? "").length, true,
            { self }
        );
        ...
        yield FSCK.check(FSCK.path_prop(path, "nameField"), `Поле имеет непустое свойство nameField`,
            !!String(self_settings.nameField ?? "").length, true,
            { self }
        );

        // db_type              : self_settings.type,                 // Тип поля
        yield FSCK.check(FSCK.path_prop(path, "type"), `Поле имеет корректное свойство type`, // см. DDL / db_type_meta2real
            self_settings.type ?? null, FSCK.is.any(["uuid", "text", "string", "integer", "float", "date", "datetime", "boolean", "ref"]),
            { self },
        );
        ...
        if (is_pseudo) {
            yield FSCK.check(FSCK.path_prop(path, ["calculated", "virtual"], "calc/virt"), `Поле не должно быть одновременно виртуальным и вычисляемым`,
                /* !is_pseudo || */ !(self_settings.calculated && self_settings.virtual), true,
                { self }
            );
            yield FSCK.check(FSCK.path_prop(path, "fnfield"), `Для виртуального/вычисляемого поля должно быть указано значение`,
                !!String(self_settings.fnfield ?? "").trim().length, true,
                { self }
            );
        }
