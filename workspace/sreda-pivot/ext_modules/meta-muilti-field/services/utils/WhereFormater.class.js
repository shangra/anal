...
class WhereFormater {
            ...
            if (name.charAt(0) !== '$') {
                ...
                if (Array.isArray(value)) {
                    ...
                    if (dictionaryValues.length > 0)
                        dictionaryParams[name] = dictionaryValues;
        ...
        for (const name in whereObj) {
            ...
            if (name.charAt(0) !== '$') {
                if (params.includes(name)) {
                    newParams['$and'] = Object.values(conditions[name]).map(
                        (val) => ({
                            [`${val}`]: value,
                        })
                    );
                ...
                if (Array.isArray(value)) {
                    const newValues = [];
                    for (const item of value) {
                        const values = this.getHavingParams(
                            item,
                            params,
                            conditions
                        );
    ...
    replaceHavingParams(whereObj, attributesAggr, flatWhere) {
        ...
        const aggrField = attributesAggr.map((attr) => {
            ...
            aggrConditions[attr.field][
                attr.alias
            ] = `$${attr.func}("${attr.field}")$`;
            return attr.field;
        });

        const having = this.getHavingParams(
            whereObj,
            aggrField,
            aggrConditions
        );
    ...
    compressFlat(whereObj) {
        ...
        whereObj.forEach((option) => {
            Object.keys(option).forEach((params) => {
                ...
                if (
                    Array.isArray(newWhere[params]['$or']) &&
                    !newWhere[params]['$or'].length
                ) {
                    delete newWhere[params]['$or'];
                }

                if (
                    Array.isArray(newWhere[params]['$and']) &&
                    !newWhere[params]['$and'].length
                ) {
        ...
        for (const param in newWhere) {
            if (
                ...
                if (
                    Array.isArray(newWhere[param]) &&
                    newWhere[param].length <= 1
                ) {
        ...
        for (const attributeName in whereObj) {
            if (Array.isArray(whereObj[attributeName])) {
                whereObj[attributeName] = whereObj[attributeName].filter(
                    (attr) =>
                        !isNil(attr) &&
                        !listFields.includes(Object.keys(attr)?.[0])
                );
                // eslint-disable-next-line guard-for-in
                for (const attrItem in whereObj[attributeName]) {
                    // eslint-disable-next-line no-await-in-loop
                    const tempWhere = this.clear(
                        whereObj[attributeName][attrItem],
                        listFields
                    );
    ...
    dictionary(whereObj) {
        ...
        for (const fieldName in dictionaryWhere) {
            ...
            dictionaryFields[fn][dictionaryFieldName] =
                dictionaryWhere[fieldName];
        }
        ...
        for (const name in whereObj) {
            ...
            if (name.charAt(0) !== '$') {
                if (params[name]?.[value])
                    newParams[name] = params[name]?.[value].where;
